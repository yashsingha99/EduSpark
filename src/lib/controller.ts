...
// const NEXT_PUBLIC_LIVEKIT_API_KEY="APIvJUuCdqyfNBe"
// const NEXT_PUBLIC_LIVEKIT_API_SECRET="oyog77WjszMSaLUfoIQfpSg0RTkW4SxNePX2bHVfSf5H"
...
export function getSessionFromReq(req: Request): Session {
  ...
  const verified = jwt.verify(token, process.env.NEXT_PUBLIC_LIVEKIT_API_SECRET!);
  if (!verified) {
...
export class Controller {
  ...
  constructor() {
    const httpUrl = process.env.NEXT_PUBLIC_LIVEKIT_WS_URL!.replace("wss://", "https://")
      .replace("ws://", "http://");
    ...
    this.egressService = new EgressClient(httpUrl);
  }
  ...
  async joinStream({
    ...
    let exists = false;
    try {
      const res = await this.roomService.getParticipant(room_name, identity);
      exists = true;
      console.log("helsdm ,===============================l", res);
  ...
  async startRecording(room_name: string, trackID: string) {
    const info = await this.egressService.startTrackEgress(
      room_name,
      'wss://digitaltutor-907f291d.livekit.cloud',
      trackID,
    );
